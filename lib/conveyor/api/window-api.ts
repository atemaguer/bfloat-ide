...
export class WindowApi extends ConveyorApi {
  ...
  windowIsFullscreen = () => this.invoke('window-is-fullscreen')
  onFullscreenChange = (callback: (isFullscreen: boolean) => void) =>
    this.on<boolean>('window-fullscreen-changed', callback)
  webOpenUrl = (url: string) => this.invoke('web-open-url', url)
  windowSetBackgroundColor = (color: string) => this.invoke('window-set-background-color', color)
}
