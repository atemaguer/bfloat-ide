...
pub async fn get_saved_server_url(app: &tauri::AppHandle) -> Option<String> {
    if let Some(url) = get_default_server_url(app.clone()).ok().flatten() {
        tracing::info!(%url, "Using desktop-specific custom server URL");
        return Some(url);
    }
        ...
        let ready = async {
            loop {
                ...
                if check_health(&url, Some(&password)).await {
                    tracing::info!(elapsed = ?timestamp.elapsed(), "Bfloat server ready");
                    return Ok(());
                }
            }
        };
...
pub async fn check_health(url: &str, password: Option<&str>) -> bool {
    ...
    let Ok(health_url) = url.join("/health") else {
        return false;
    };
    ...
    if let Some(password) = password {
        req = req.basic_auth("bfloat", Some(password));
    }
...
pub async fn check_health_or_ask_retry(app: &AppHandle, url: &str) -> bool {
    tracing::debug!(%url, "Checking health of custom server");
    loop {
